...
impl PersonalityAgentBuilder {
    ...
    pub fn create_researcher(data_dir: &str, provider: &str) -> Result<Box<dyn Agent>, Error> {
        let config = AgentConfig {
            ...
            system_prompt: Some(
                ...
                \nYou actively use memory blocks to store important discoveries, facts, and research findings for future reference.".to_string()
            ),
        ...
        let memory_manager = {
            ...
            std::fs::create_dir_all(&agent_data_dir).map_err(|e| anyhow!("Failed to create agent data directory: {}", e))?;
            let memory_store = FjallMemoryStore::new(&agent_data_dir)?;
            std::sync::Arc::new(MemoryManager::new(memory_store))
        };

        let mut tools = HashMap::new();
        tools.insert("search".to_string(), Box::new(DDGSearchTool) as Box<dyn AiTool>);
        tools.insert("website".to_string(), Box::new(WebsiteTool) as Box<dyn AiTool>);
        tools.insert("block".to_string(), Box::new(BlockTool { memory_manager: memory_manager.clone() }) as Box<dyn AiTool>);
        tools.insert("retrieve_context".to_string(), Box::new(RetrieveContextTool { memory_manager: memory_manager.clone() }) as Box<dyn AiTool>);
        tools.insert("update_block".to_string(), Box::new(UpdateBlockTool { memory_manager: memory_manager.clone() }) as Box<dyn AiTool>);
    ...
    pub fn create_calculator(data_dir: &str, provider: &str) -> Result<Box<dyn Agent>, Error> {
        let config = AgentConfig {
            ...
            system_prompt: Some(
                ...
                \n\nYou think systematically, show your work, and double-check important calculations.".to_string()
            ),
            provider: provider.to_string(),
    ...
    pub fn create_coordinator(data_dir: &str, provider: &str) -> Result<Box<dyn Agent>, Error> {
        let config = AgentConfig {
            ...
            system_prompt: Some(
                ...
                \nYou actively use memory blocks to track project goals, user preferences, and important decisions.".to_string()
            ),
            provider: provider.to_string(),
    ...
    pub fn create_pragmatic(data_dir: &str, provider: &str) -> Result<Box<dyn Agent>, Error> {
        let config = AgentConfig {
            ...
            system_prompt: Some(
                ...
                \n\nYou prefer simple, working solutions over complex theoretical approaches.".to_string()
            ),
            provider: provider.to_string(),
    ...
    pub fn list_personalities() -> Vec<(&'static str, &'static str, &'static str)> {
        vec![
            ("researcher", "Dr. Research", "Thorough analyst and fact-finder"),
            ("calculator", "Logic", "Precise mathematical problem-solver"),
            ("creative", "Spark", "Imaginative and artistic thinker"),
            ("coordinator", "Maestro", "Strategic organizer and delegator"),
            ("pragmatic", "Practical", "Efficient and solution-focused"),
        ]
    }

    /// Create an agent by personality type
    pub fn create_by_type(personality: &str, data_dir: &str, provider: &str) -> Result<Box<dyn Agent>, Error> {
        match personality.to_lowercase().as_str() {
            ...
            _ => Err(anyhow!("Unknown personality type: {}. Available: researcher, calculator, creative, coordinator, pragmatic", personality))
        }
    }
}
...
impl PersonalityAgent {
    pub fn new(
        ...
        let tool_vec: Vec<Box<dyn AiTool>> = tools.values()
            .map(|tool| {
                ...
                match tool.name() {
                    ...
                    _ => Box::new(DummyTool { name: tool.name().to_string() }) as Box<dyn AiTool>,
                }
            })
            .collect();
...
impl Agent for PersonalityAgent {
    ...
    async fn process_message(&mut self, message: AgentMessage) -> Result<MessageResponse, Error> {
        debug!("Agent {} ({}) processing message from {}", self.name(), self.agent_id(), message.from_agent_id);
        debug!("Agent {} has {} tools available: {:?}", self.name(), self.tools.len(), 
                    ...
                    match response_content {
                        genai::chat::MessageContent::ToolCalls(tool_calls) => {
                            ...
                            debug!("Agent {} continuing loop after tool execution, conversation now has {} messages", 
                                   self.name(), conversation_messages.len());
                            
                            // Continue the loop to get the next LLM response
                            continue;
                        }
                        genai::chat::MessageContent::Text(response_text) => {
                            info!("Agent {} generated final response: {}", self.name(), response_text);
                        ...
                        genai::chat::MessageContent::Parts(parts) => {
                            ...
                            if !combined_text.is_empty() {
                                info!("Agent {} generated final response from parts: {}", self.name(), combined_text);
