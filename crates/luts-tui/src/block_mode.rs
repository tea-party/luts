...
use crate::{
    ...
    events::AppEvent,
    markdown::SimpleMarkdownRenderer,
};
use anyhow::Result;
use crossterm::event::{KeyCode, KeyEvent, MouseEvent, MouseEventKind};
use luts_core::memory::{
    MemoryManager, MemoryBlock, MemoryBlockBuilder, BlockType, 
    MemoryContent, BlockId, FjallMemoryStore
};
...
impl BlockMode {
    pub fn new(event_sender: mpsc::UnboundedSender<AppEvent>) -> Self {
        // Initialize memory manager with Fjall store
        let data_dir = PathBuf::from("./data");
        let fjall_store = FjallMemoryStore::new(data_dir).expect("Failed to create Fjall store");
        let memory_manager = Arc::new(MemoryManager::new(fjall_store));
        ...
        let memory_blocks = vec![
            MemoryBlockBuilder::new()
                .with_type(BlockType::Preference)
                .with_user_id(&user_id)
                .with_session_id(&session_id)
                .with_content(MemoryContent::Text("Prefers casual tone with emojis".to_string()))
                .with_tag("communication")
                .build()
                .unwrap(),
            MemoryBlockBuilder::new()
                .with_type(BlockType::PersonalInfo)
                .with_user_id(&user_id)
                .with_session_id(&session_id)
                .with_content(MemoryContent::Text("Lives in San Francisco, enjoys outdoor activities".to_string()))
                .with_tag("location")
                .build()
                .unwrap(),
            MemoryBlockBuilder::new()
                .with_type(BlockType::Goal)
                .with_user_id(&user_id)
                .with_session_id(&session_id)
                .with_content(MemoryContent::Text("Build a social media presence for weather updates".to_string()))
                .with_tag("social_media")
                .build()
                .unwrap(),
        ];
                ...
                if self.focused_panel == FocusedPanel::List {
                    if let Some(selected) = self.block_list_state.selected() {
                        if let Some(block) = self.memory_blocks.get(selected) {
                            ...
                            self.editor_content = block.content().as_text().unwrap_or("").to_string();
                    ...
                    if let Some(selected) = self.block_list_state.selected() {
                        if selected < self.memory_blocks.len() {
                            ...
                            if self.memory_blocks.is_empty() {
                                ...
                                self.block_list_state.select(Some(self.memory_blocks.len() - 1));
                            }
                        }
                    }
                }
        ...
        match key.code {
            ...
            KeyCode::Enter => {
                if !self.create_dialog_input.trim().is_empty() {
                    ...
                    info!("Created new {} block with content: {}", self.create_dialog_type, self.create_dialog_input);
                }
                self.show_create_dialog = false;
                self.create_dialog_input.clear();
            }
        ...
        match key.code {
            ...
            KeyCode::Right => {
                if self.editor_cursor_pos < self.editor_content.len() {
                    // Handle multi-byte UTF-8 characters properly
                    let mut pos = self.editor_cursor_pos;
                    while pos < self.editor_content.len() && !self.editor_content.is_char_boundary(pos + 1) {
                        pos += 1;
                    }
    ...
    fn move_cursor_up(&mut self) {
        ...
        if line_start > 0 {
            ...
            let prev_line_start = self.editor_content[..prev_line_end].rfind('\n')
                .map(|pos| pos + 1)
                .unwrap_or(0);
    ...
    fn move_cursor_down(&mut self) {
        ...
        if line_end < self.editor_content.len() {
            ...
            let next_line_end = self.editor_content[next_line_start..].find('\n')
    ...
    fn render_editor_content_with_cursor(&self) -> Vec<Line<'static>> {
        ...
        for (line_idx, line_text) in text_lines.iter().enumerate() {
            if line_idx == cursor_line {
                ...
                if cursor_col == 0 {
                    ...
                    spans.push(Span::styled("█", Style::default().bg(Color::White).fg(Color::Black))); // Block cursor
                    spans.push(Span::styled(line_text.to_string(), Style::default().fg(Color::White)));
                    ...
                    spans.push(Span::styled(at_cursor.to_string(), Style::default().bg(Color::White).fg(Color::Black))); // Block cursor
                ...
                lines.push(Line::from(Span::styled(line_text.to_string(), Style::default().fg(Color::White))));
            }
        }
        ...
        if text_lines.is_empty() || (cursor_line >= text_lines.len()) {
            lines.push(Line::from(Span::styled("█", Style::default().bg(Color::White).fg(Color::Black))));
        }
    ...
    fn render_block_list(&mut self, frame: &mut Frame, area: Rect) {
        ...
        let items: Vec<ListItem> = self.memory_blocks
            .iter()
            .map(|block| {
                ...
                let content_preview = block.content().as_text()
                    .map(|text| {
                        if text.len() > 50 {
                            format!("{}...", &text[..47])
                        } else {
                ...
                let content = Line::from(vec![
                    Span::styled(format!("[{}] ", type_str), Style::default().fg(color).add_modifier(Modifier::BOLD)),
    ...
    fn render_block_details(&self, frame: &mut Frame, area: Rect) {
        ...
        let selected_block = self.block_list_state.selected()
        ...
        let content = if let Some(block) = selected_block {
            ...
            vec![
                ...
                Line::from(vec![
                    ...
                    Span::styled(format!("{}", block.block_type()), Style::default().fg(Color::Yellow)),
                ]),
                Line::from(vec![
                ...
                Line::from(vec![
                    ...
                    Span::styled(block.session_id().unwrap_or("None"), Style::default().fg(Color::White)),
                ]),
                Line::from(vec![
                    Span::styled("Created: ", Style::default().fg(Color::Cyan)),
                    Span::styled(format!("{}", block.created_at()), Style::default().fg(Color::Gray)),
                ]),
                Line::from(vec![
                    Span::styled("Updated: ", Style::default().fg(Color::Cyan)),
                    Span::styled(format!("{}", block.updated_at()), Style::default().fg(Color::Gray)),
                ]),
                Line::from(vec![
    ...
    fn render_workflow_status(&self, frame: &mut Frame, area: Rect) {
        ...
        let content = if let Some(block_id) = &self.editing_block {
            vec![
                Line::from(vec![
                    Span::styled("Editing Block: ", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
                    Span::styled(block_id.as_str(), Style::default().fg(Color::Yellow)),
                ]),
                Line::from(""),
                Line::from(vec![
                    Span::styled("Content (Ctrl+S to save, Esc to cancel):", Style::default().fg(Color::Gray)),
                ]),
                Line::from(""),
            ]
        } else {
            let total_blocks = self.memory_blocks.len();
            let message_count = self.memory_blocks.iter().filter(|b| b.block_type() == BlockType::Message).count();
            let fact_count = self.memory_blocks.iter().filter(|b| b.block_type() == BlockType::Fact).count();
            let preference_count = self.memory_blocks.iter().filter(|b| b.block_type() == BlockType::Preference).count();
            let goal_count = self.memory_blocks.iter().filter(|b| b.block_type() == BlockType::Goal).count();

            vec![
                Line::from(vec![
                    Span::styled("Memory Block Editor", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
                ]),
                Line::from(""),
                Line::from(vec![
                    Span::styled("📊 Total Blocks: ", Style::default().fg(Color::Blue)),
                    Span::styled(format!("{}", total_blocks), Style::default().fg(Color::White)),
                ]),
                Line::from(vec![
                    Span::styled("💬 Messages: ", Style::default().fg(Color::Green)),
                    Span::styled(format!("{}", message_count), Style::default().fg(Color::White)),
                ...
                Line::from(vec![
                    ...
                    Span::styled(format!("{}", preference_count), Style::default().fg(Color::White)),
                ...
                Line::from(vec![
                    Span::styled("Enter=edit, Del=delete, Ctrl+n=create, Ctrl+s=save, Ctrl+r=refresh", Style::default().fg(Color::Gray)),
                ]),
            ]
        };
